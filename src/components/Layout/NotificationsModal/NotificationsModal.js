...
const NotificationsModal = ({ onClose }) => {
  ...
  const lowStockProducts = state.products.filter(p => (p.stock || 0) <= state.lowStockThreshold);
  ...
  const notifications = [
    ...lowStockProducts.map(product => ({
      ...
      message: `${product.name} ${getTranslation('isRunningLow', state.currentLanguage)} (${product.stock || 0} ${getTranslation('unitsRemaining', state.currentLanguage)})`,
      icon: Package,
      color: 'yellow'
    })),
