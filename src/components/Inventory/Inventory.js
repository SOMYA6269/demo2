...
const Inventory = () => {
  ...
  const filteredProducts = state.products.filter(product => {
    ...
    const matchesStatus = !filterStatus || (
      filterStatus === 'low-stock' && (product.stock || 0) <= state.lowStockThreshold ||
  ...
  const lowStockProducts = state.products.filter(p => (p.stock || 0) <= state.lowStockThreshold).length;
  const outOfStockProducts = state.products.filter(p => (p.stock || 0) === 0).length;
  ...
  const totalValue = state.products.reduce((sum, p) => sum + ((p.stock || 0) * (p.price || 0)), 0);
  ...
  const adjustStock = (productId, adjustment) => {
    ...
    if (product) {
      ...
      dispatch({ type: 'ADD_ACTIVITY', payload: {
        ...
        message: `Stock adjusted for ${product.name}: ${adjustment > 0 ? '+' : ''}${adjustment}`,
  ...
  const exportInventory = () => {
    const csvContent = [
      ['Name', 'Category', 'Stock', 'Price', 'Value', 'Status', 'Barcode', 'Expiry Date'],
              ...
              {paginatedProducts.map((product) => {
                ...
                return (
                  <tr key={product.id} className="hover:bg-blue-50 transition-colors">
                    ...
                    <td className="font-semibold">{product.stock || 0}</td>
                    <td className="font-semibold">₹{product.price || 0}</td>
